...
const generateCode = (node: GripDistanceProgramNode, scriptContext: ScriptContext, applicationContext: ApplicationContext): ScriptBuilder => {
    const applicationNode = applicationContext.contributions.getContribution('ur-sample-gripdistance-application') as GripDistanceProgramNode;
    ...
    if (node.parameters && Object.prototype.hasOwnProperty.call(node.parameters, 'gripperToggle')) {
        const gripDistance = node.parameters.gripperToggle
            ? applicationNode.parameters.openDistance.value
            : applicationNode.parameters.closedDistance.value;
